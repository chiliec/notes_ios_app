...
class NoteExtensionsTests: XCTestCase {
    ...
    func testNoteExtensions_whenGetJsonWithNormalImportant_hasNotSaveImportant() {
        let note = Note(title: "Text", content: "More", importance: .ordinary)
        let json = sut.json
    ...
    func testNoteExtensions_whenGetJsonAndParseJsonForFullNote_isEqualsNotes() {
        let originNote = Note(uid: UUID().uuidString, title: "Title1", content: "My text", color: .red, importance: .unimportant, selfDestructionDate: Date())
